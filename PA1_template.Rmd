...
We notice by looking at the bar plot that for some days no data are available. 
We now plot the following histogram where the frequency of days for number of steps is shown:
...
As previously remarked, the data set contains some "NA" value. The purpose is now cleaning the data set by removing the invalid values.
We start by checking which row contains an invalid value:
...
We substitute those 'NA' values with the mean of the corresponding time interval computed on the entire data set and we re-calculate the mean and the median on the new data set:
